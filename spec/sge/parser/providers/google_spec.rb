...
RSpec.describe SGE::Parser::Providers::Google do
  ...
  describe "#search" do
    it "navigates to Google search with gbv=1" do
      expect(browser).to receive(:go_to).with(
        a_string_starting_with("https://www.google.com/search?")
        .and(include("gbv=1"))
        .and(include("q=turing+machine"))
      )
    ...
    it "raises CaptchaError when CAPTCHA is detected" do
      ...
      expect {
        provider.search(browser, "turing machine")
      }.to raise_error(SGE::Parser::CaptchaError)
    end
